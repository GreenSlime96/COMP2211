...
public abstract class ImmutableQHashSeparateKVByteShortMapSO
        extends ImmutableQHashSeparateKVByteKeyMap
        implements HashByteShortMap, InternalByteShortMapOps, SeparateKVByteShortQHash {

    short[] values;

    void copy(SeparateKVByteShortQHash hash) {
        super.copy(hash);
        values = hash.valueArray().clone();
    }

    void move(SeparateKVByteShortQHash hash) {
        super.move(hash);
