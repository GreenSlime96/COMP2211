...
abstract class ByteQHashFactory
        <MT> extends ByteHashFactorySO {

    ByteQHashFactory(HashConfig hashConf, int defaultExpectedSize
    ...
    MT newMutableHash(int expectedSize) {
        ...
        if (randomRemoved) {
            ...
            removed = (byte) random.nextInt();
            if (randomFree) {
                free = (byte) random.nextInt();
            } else {
                free = freeValue;
            }
            while (free == removed) {
