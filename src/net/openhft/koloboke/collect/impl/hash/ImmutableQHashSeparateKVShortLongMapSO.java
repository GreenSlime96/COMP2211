...
public abstract class ImmutableQHashSeparateKVShortLongMapSO
        extends ImmutableQHashSeparateKVShortKeyMap
        implements HashShortLongMap, InternalShortLongMapOps, SeparateKVShortLongQHash {

    long[] values;

    void copy(SeparateKVShortLongQHash hash) {
        super.copy(hash);
        values = hash.valueArray().clone();
    }

    void move(SeparateKVShortLongQHash hash) {
        super.move(hash);
