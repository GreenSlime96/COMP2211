...
public abstract class ImmutableLHashSeparateKVByteFloatMapSO
        extends ImmutableLHashSeparateKVByteKeyMap
        implements HashByteFloatMap, InternalByteFloatMapOps, SeparateKVByteFloatLHash {

    int[] values;

    void copy(SeparateKVByteFloatLHash hash) {
        super.copy(hash);
        values = hash.valueArray().clone();
    }

    void move(SeparateKVByteFloatLHash hash) {
        super.move(hash);
