...
public abstract class ImmutableLHashSeparateKVIntShortMapSO
        extends ImmutableLHashSeparateKVIntKeyMap
        implements HashIntShortMap, InternalIntShortMapOps, SeparateKVIntShortLHash {

    short[] values;

    void copy(SeparateKVIntShortLHash hash) {
        super.copy(hash);
        values = hash.valueArray().clone();
    }

    void move(SeparateKVIntShortLHash hash) {
        super.move(hash);
