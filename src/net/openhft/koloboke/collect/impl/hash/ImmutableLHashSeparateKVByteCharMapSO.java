...
public abstract class ImmutableLHashSeparateKVByteCharMapSO
        extends ImmutableLHashSeparateKVByteKeyMap
        implements HashByteCharMap, InternalByteCharMapOps, SeparateKVByteCharLHash {

    char[] values;

    void copy(SeparateKVByteCharLHash hash) {
        super.copy(hash);
        values = hash.valueArray().clone();
    }

    void move(SeparateKVByteCharLHash hash) {
        super.move(hash);
