...
public abstract class UpdatableQHashSeparateKVShortFloatMapSO
        extends UpdatableQHashSeparateKVShortKeyMap
        implements HashShortFloatMap, InternalShortFloatMapOps, SeparateKVShortFloatQHash {

    int[] values;

    void copy(SeparateKVShortFloatQHash hash) {
        super.copy(hash);
        values = hash.valueArray().clone();
    }

    void move(SeparateKVShortFloatQHash hash) {
        super.move(hash);
