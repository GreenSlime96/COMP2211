...
public abstract class ImmutableQHashSeparateKVCharLongMapSO
        extends ImmutableQHashSeparateKVCharKeyMap
        implements HashCharLongMap, InternalCharLongMapOps, SeparateKVCharLongQHash {

    long[] values;

    void copy(SeparateKVCharLongQHash hash) {
        super.copy(hash);
        values = hash.valueArray().clone();
    }

    void move(SeparateKVCharLongQHash hash) {
        super.move(hash);
