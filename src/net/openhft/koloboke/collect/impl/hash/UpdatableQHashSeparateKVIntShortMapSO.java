...
public abstract class UpdatableQHashSeparateKVIntShortMapSO
        extends UpdatableQHashSeparateKVIntKeyMap
        implements HashIntShortMap, InternalIntShortMapOps, SeparateKVIntShortQHash {

    short[] values;

    void copy(SeparateKVIntShortQHash hash) {
        super.copy(hash);
        values = hash.valueArray().clone();
    }

    void move(SeparateKVIntShortQHash hash) {
        super.move(hash);
