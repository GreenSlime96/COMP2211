...
public abstract class ImmutableLHashSeparateKVDoubleObjMapSO<V>
        extends ImmutableLHashSeparateKVDoubleKeyMap
        implements HashDoubleObjMap<V>,
        InternalDoubleObjMapOps<V>, SeparateKVDoubleObjLHash {

    V[] values;

    void copy(SeparateKVDoubleObjLHash hash) {
        super.copy(hash);
        // noinspection unchecked
        values = (V[]) hash.valueArray().clone();
    }

    void move(SeparateKVDoubleObjLHash hash) {
        super.move(hash);
        // noinspection unchecked
