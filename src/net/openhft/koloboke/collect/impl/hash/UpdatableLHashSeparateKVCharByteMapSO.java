...
public abstract class UpdatableLHashSeparateKVCharByteMapSO
        extends UpdatableLHashSeparateKVCharKeyMap
        implements HashCharByteMap, InternalCharByteMapOps, SeparateKVCharByteLHash {

    byte[] values;

    void copy(SeparateKVCharByteLHash hash) {
        super.copy(hash);
        values = hash.valueArray().clone();
    }

    void move(SeparateKVCharByteLHash hash) {
        super.move(hash);
