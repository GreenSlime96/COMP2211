...
public abstract class QHashParallelKVObjObjMapFactorySO<K, V>
        extends ObjHashFactorySO<K>
        implements HashObjObjMapFactory<K, V> {

    QHashParallelKVObjObjMapFactorySO(HashConfig hashConf, int defaultExpectedSize,
    ...
    public <K2 extends K, V2 extends V> UpdatableQHashParallelKVObjObjMapGO<K2, V2> newUpdatableMap(
            int expectedSize) {
        UpdatableQHashParallelKVObjObjMapGO<K2, V2> map = uninitializedUpdatableMap();
        map.init(configWrapper, expectedSize);
        return map;
    }
    ...
    public <K2 extends K, V2 extends V> UpdatableQHashParallelKVObjObjMapGO<K2, V2> newUpdatableMap(
        ...
        if (map instanceof ObjObjMap) {
            ...
            if (map instanceof ParallelKVObjObjQHash) {
                ParallelKVObjObjQHash hash = (ParallelKVObjObjQHash) map;
                if (hash.hashConfig().equals(hashConf) &&
                        objObjMap.keyEquivalence().equals(getKeyEquivalence())) {
                    UpdatableQHashParallelKVObjObjMapGO<K2, V2> res =
                            uninitializedUpdatableMap();
                    res.copy(hash);
                    return res;
                }
            }
            UpdatableQHashParallelKVObjObjMapGO<K2, V2> res = newUpdatableMap(expectedSize);
            res.putAll(map);
            return res;
        } else {
            UpdatableQHashParallelKVObjObjMapGO<K2, V2> res = newUpdatableMap(expectedSize);
