...
public abstract class QHashSeparateKVObjDoubleMapFactorySO<K>
        extends ObjHashFactorySO<K>
        implements HashObjDoubleMapFactory<K> {

    QHashSeparateKVObjDoubleMapFactorySO(HashConfig hashConf, int defaultExpectedSize,
    ...
    <K2 extends K>
     MutableQHashSeparateKVObjDoubleMapGO<K2>
    uninitializedMutableMap() {
        return new MutableQHashSeparateKVObjDoubleMap<K2>();
    }
    <K2 extends K>
     UpdatableQHashSeparateKVObjDoubleMapGO<K2>
    uninitializedUpdatableMap() {
        return new UpdatableQHashSeparateKVObjDoubleMap<K2>();
    }
    <K2 extends K>
     ImmutableQHashSeparateKVObjDoubleMapGO<K2>
    uninitializedImmutableMap() {
        return new ImmutableQHashSeparateKVObjDoubleMap<K2>();
    }
    ...
    public <K2 extends K>
     UpdatableQHashSeparateKVObjDoubleMapGO<K2> newUpdatableMap(
            int expectedSize) {
        UpdatableQHashSeparateKVObjDoubleMapGO<K2> map = uninitializedUpdatableMap();
        map.init(configWrapper, expectedSize);
        return map;
    }
    ...
    public <K2 extends K>
     UpdatableQHashSeparateKVObjDoubleMapGO<K2> newUpdatableMap(
        ...
        if (map instanceof ObjDoubleMap) {
            ...
            if (map instanceof SeparateKVObjDoubleQHash) {
                SeparateKVObjDoubleQHash hash = (SeparateKVObjDoubleQHash) map;
                if (hash.hashConfig().equals(hashConf) &&
                        objDoubleMap.keyEquivalence().equals(getKeyEquivalence())) {
                    UpdatableQHashSeparateKVObjDoubleMapGO<K2> res =
                            uninitializedUpdatableMap();
                    res.copy(hash);
                    return res;
                }
            }
            UpdatableQHashSeparateKVObjDoubleMapGO<K2> res = newUpdatableMap(expectedSize);
            res.putAll(map);
            return res;
        } else {
            UpdatableQHashSeparateKVObjDoubleMapGO<K2> res = newUpdatableMap(expectedSize);
