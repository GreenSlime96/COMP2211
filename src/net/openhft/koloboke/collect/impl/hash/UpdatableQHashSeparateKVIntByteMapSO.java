...
public abstract class UpdatableQHashSeparateKVIntByteMapSO
        extends UpdatableQHashSeparateKVIntKeyMap
        implements HashIntByteMap, InternalIntByteMapOps, SeparateKVIntByteQHash {

    byte[] values;

    void copy(SeparateKVIntByteQHash hash) {
        super.copy(hash);
        values = hash.valueArray().clone();
    }

    void move(SeparateKVIntByteQHash hash) {
        super.move(hash);
