...
public abstract class ImmutableLHashSeparateKVFloatByteMapSO
        extends ImmutableLHashSeparateKVFloatKeyMap
        implements HashFloatByteMap, InternalFloatByteMapOps, SeparateKVFloatByteLHash {

    byte[] values;

    void copy(SeparateKVFloatByteLHash hash) {
        super.copy(hash);
        values = hash.valueArray().clone();
    }

    void move(SeparateKVFloatByteLHash hash) {
        super.move(hash);
