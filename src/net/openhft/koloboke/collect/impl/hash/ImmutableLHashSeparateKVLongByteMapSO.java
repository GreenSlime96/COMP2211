...
public abstract class ImmutableLHashSeparateKVLongByteMapSO
        extends ImmutableLHashSeparateKVLongKeyMap
        implements HashLongByteMap, InternalLongByteMapOps, SeparateKVLongByteLHash {

    byte[] values;

    void copy(SeparateKVLongByteLHash hash) {
        super.copy(hash);
        values = hash.valueArray().clone();
    }

    void move(SeparateKVLongByteLHash hash) {
        super.move(hash);
